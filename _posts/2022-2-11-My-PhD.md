...
The demagnetization field, which can be derived from the Maxwell equation (checkout [my theis](https://theses.hal.science/tel-03770225/document) for the derivation), can be expressed as:

B_\text{demag}(\mathbf{r}) = \int \frac{\rho_m(\mathbf{r'})}{|\mathbf{r} - \mathbf{r'}|^3} (\mathbf{r} - \mathbf{r'}) d\mathbf{r'}
