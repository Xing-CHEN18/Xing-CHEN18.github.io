...
<p align="center">
 ...
 Atomic scale representation of individual magnetic moments \\(\boldsymbol{\mu}_i\\). b) Micromagnetic scale representation of the magnetization vector \\(\boldsymbol{M}\\) defining as the sum of all magnetic moments \\(\bm{\mu}_i\\) inside the volume \\(dV\\): \\(\boldsymbol{M} = \sum_{i} \boldsymbol{\mu}_i/dV\\).
